...
export const adrsService = {
  listProjectAdrs(projectId: string) {
  ...
  async createAdrDraftJob(
    projectId: string,
    input: Omit<AdrDraftInput, "project">,
  ) {
  ...
  async getAdr(adrId: string) {
    return prisma.adr.findUnique({ where: { id: adrId } });
  },

  async createAdrReviewJob(adrId: string) {
  ...
  async createProjectHealthCheckJob(projectId: string) {
    const project = await prisma.project.findUnique({ where: { id: projectId } });
  ...
  getHealthCheck(healthCheckId: string) {
    return prisma.projectHealthCheck.findUnique({ where: { id: healthCheckId } });
  },

  async getLatestHealthCheck(projectId: string) {
