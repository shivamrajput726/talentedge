...
export function registerRoutes(app) {
    app.get("/health", async (_req, res) => {
        const health = { status: "ok", db: "ok", redis: "ok" };
        try {
    ...
    app.get("/api/projects/:projectId/adrs", async (req, res) => {
        const params = z
    ...
    app.get("/api/adrs/:adrId", async (req, res) => {
    ...
    app.post("/api/projects/:projectId/adrs/draft", async (req, res) => {
        const params = z
            .object({ projectId: z.string().uuid() })
            .safeParse(req.params);
        if (!params.success) {
    ...
    app.post("/api/adrs/:adrId/reviews", async (req, res) => {
        const params = z.object({ adrId: z.string().uuid() }).safeParse(req.params);
        if (!params.success) {
            return res.status(400).json({ error: params.error.flatten() });
        }
    ...
    app.post("/api/projects/:projectId/health-checks", async (req, res) => {
        const params = z
            .object({ projectId: z.string().uuid() })
            .safeParse(req.params);
        if (!params.success) {
        ...
        const healthCheck = await adrsService.createProjectHealthCheckJob(params.data.projectId);
        if (!healthCheck) {
            return res.status(404).json({ error: "Project not found" });
        }
        return res.status(201).json(healthCheck);
    });
    app.get("/api/projects/:projectId/health-checks/latest", async (req, res) => {
        const params = z
            .object({ projectId: z.string().uuid() })
            .safeParse(req.params);
        if (!params.success) {
        ...
        const healthCheck = await adrsService.getLatestHealthCheck(params.data.projectId);
        if (!healthCheck)
