...
function stubDelay(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
export const aiService = {
    async generateAdrDraft(input) {
        ...
        const response = await client.responses.create({
            ...
            instructions: "You are an experienced software architect helping a team capture high-quality Architecture Decision Records (ADRs).",
            input: prompt,
        });
    ...
    async reviewAdr(adrText) {
        if (env.AI_PROVIDER === "stub") {
            await stubDelay(500);
            return {
                summary: "Clear ADR structure with a reasonable decision, but it could use tighter success criteria and a rollback plan.",
        ...
        const response = await client.responses.create({
            model: env.OPENAI_MODEL,
            instructions: "You are an experienced software architect reviewing an Architecture Decision Record (ADR).",
            input: [
    ...
    async runProjectHealthCheck(input) {
        if (env.AI_PROVIDER === "stub") {
            await stubDelay(800);
            return {
                summary: "Healthy foundation. Biggest gaps: explicit security ADR and observability decision.",
        ...
        const response = await client.responses.create({
            model: env.OPENAI_MODEL,
            instructions: "You are a principal engineer doing a pragmatic architecture review. You identify contradictions, missing decisions, and next steps.",
            input: [
                `Project: ${input.project.name}`,
